...
def evaluate_barriers_optimized(paths: np.ndarray, sigma: float, pt_grid: np.ndarray, sl_grid: np.ndarray):
    ...
    # Avoid np.ones allocation completely by using a constant column
    # padding_col = np.ones((num_paths, 1), dtype=bool) is slow
    # but we can reuse the same array.
    # Actually, argmax can be optimized differently:
    # Instead of padding with ones, we can just find argmax of hits.
    # But argmax of all False gives 0, which is incorrect (we want it to act as if it hit at length).
    # So we need to handle paths that never hit.

    # We can pre-allocate padding_col once
    ...
    for i, pt_level in enumerate(pt_levels):
        ...
        # Avoid hstack, just allocate
        hit_pt_padded = np.empty((num_paths, length + 1), dtype=bool)
        hit_pt_padded[:, :-1] = hit_pt
        hit_pt_padded[:, -1] = True
    ...
    for j, sl_level in enumerate(sl_levels):
        ...
        hit_sl_padded = np.empty((num_paths, length + 1), dtype=bool)
        hit_sl_padded[:, :-1] = hit_sl
        hit_sl_padded[:, -1] = True
