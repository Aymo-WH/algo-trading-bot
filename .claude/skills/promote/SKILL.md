...
1. Ask the operator to retrieve the token themselves (it was written at lockbox build
   ...
   `! /workspace/venv/bin/python validation/final_eval.py --config <candidate-config> --operator-token <TOKEN>`
...
3. Report the result honestly — pass or fail. **There is no second evaluation.** If it
