...
def evaluate_barriers_optimized(paths: np.ndarray, sigma: float, pt_grid: np.ndarray, sl_grid: np.ndarray):
    ...
    # Pre-allocate padded hits to avoid repeated hstack allocation inside loop
    # shape: (num_paths, length + 1)
    # The last column is always True (hit at length)
...
print(f"More Optimized Time: {end_time - start_time:.4f} seconds")
