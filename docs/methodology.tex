...
\section{The Meta-Labeling Framework (V2 Architecture)}
A fundamental flaw in predictive modeling is conflating trade direction with conviction. We separated these concerns via a dual-agent meta-labeling architecture. The V2 architecture implements XGBoost for robust classification and PPO for meta-labeling risk management:
\begin{itemize}
    \item \textbf{Primary Model (XGBoost):} Trained using the Triple-Barrier Method to predict directional class labels $c \in \{0, 1, 2\}$, mapping directly to signals (-1.0, 0.0, 1.0).
    \item \textbf{Secondary Model (PPO):} Receives a strictly constrained meta-feature state vector and outputs a continuous bet size $m \in [0.0, 1.0]$. The PPO agent strictly handles sizing and is structurally prevented from counter-trading the primary signal.
\end{itemize}

\subsection{Safe RL Reward Function}
The `TradingEnv` is formulated as a Safe RL environment to enforce responsible bet-sizing by the PPO agent. The reward $R_t$ is computed not just as profit, but as Base Gross Profit minus several structured penalties:
\begin{itemize}
    \item \textbf{Turnover Penalty:} Transaction fees proportionally scaled by the bet size $m$.
    \item \textbf{Variance Penalty:} An ATR-based volatility penalty scaled by the squared bet size $m^2$, penalizing large bets in volatile conditions.
    \item \textbf{CVaR Penalty:} A catastrophic drawdown penalty that triggers severely if portfolio drawdown exceeds 5\%.
\end{itemize}
This formulation forces the PPO meta-labeler to act strictly as a risk manager rather than a profit-maximizer.
...
\section{Architecture V2.0 Upgrades}
\begin{itemize}
    \item \textbf{Microstructural Shift:} Completely removed retail technical indicators, replacing them with institutionally robust microstructure signals: SADF, VPIN, Amihud's Illiquidity, and Kyle's Lambda.
